...
function settings(PDO $pdo): array {
    ...
    $r = [];
    foreach ($rows as $row) $r[$row['skey']] = $row['sval'];
    return $r;
}
...
// ---- GET PUBLIC (untuk index.php) ----
case 'get_public':
    $s = settings($pdo);
    $guru   = $pdo->query("SELECT id,nama,jabatan,foto FROM guru ORDER BY urutan,id")->fetchAll();
    $slider = $pdo->query("SELECT id,gambar FROM slider ORDER BY urutan,id")->fetchAll();
    $galeri = $pdo->query("SELECT id,gambar FROM galeri ORDER BY urutan,id")->fetchAll();
    $berita = $pdo->query("SELECT id,judul,tanggal,kategori,isi FROM berita ORDER BY id DESC")->fetchAll();
    ...
    echo json_encode([
        ...
        'galeri' => array_column($galeri, 'gambar'),
        'berita' => $berita,
...
case 'login':
    ...
    $user = $pdo->prepare("SELECT * FROM users WHERE username=? AND status='aktif'");
...
case 'delete_guru':
    requireAuth();
    $id = (int)($input['id'] ?? 0);
    $g = $pdo->prepare("SELECT nama FROM guru WHERE id=?"); $g->execute([$id]); $row = $g->fetch();
    $pdo->prepare("DELETE FROM guru WHERE id=?")->execute([$id]);
    addLog($pdo, 'Hapus Guru', 'Menghapus guru: '.($row['nama']??''));
    echo json_encode(['ok'=>true]);
    break;
...
case 'add_galeri':
    ...
    $gambar = $input['gambar'] ?? '';
    if (!$gambar) { echo json_encode(['error'=>'Gambar kosong']); break; }
    $pdo->prepare("INSERT INTO galeri (gambar) VALUES (?)")->execute([$gambar]);
    addLog($pdo, 'Upload Galeri', 'Menambah foto galeri');
    echo json_encode(['ok'=>true]);
    break;
...
case 'get_galeri':
    requireAuth();
    echo json_encode($pdo->query("SELECT id,gambar FROM galeri ORDER BY urutan,id")->fetchAll());
    break;
...
case 'edit_berita':
    ...
    addLog($pdo, 'Edit Berita', "Mengedit berita: $judul");
    echo json_encode(['ok'=>true]);
    break;
...
case 'delete_berita':
    requireAuth();
    $id = (int)($input['id'] ?? 0);
    $b = $pdo->prepare("SELECT judul FROM berita WHERE id=?"); $b->execute([$id]); $row = $b->fetch();
    $pdo->prepare("DELETE FROM berita WHERE id=?")->execute([$id]);
    addLog($pdo, 'Hapus Berita', 'Menghapus berita: '.($row['judul']??''));
    echo json_encode(['ok'=>true]);
    break;
...
case 'add_ppdb':
    $data = $input;
    $pdo->prepare("INSERT INTO ppdb (nama,jk,tempat,tgl,alamat,hp,jalur,ijazah,akte,tanggal_daftar) VALUES (?,?,?,?,?,?,?,?,?,?)")
        ->execute([$data['nama']??'',$data['jk']??'',$data['tempat']??'',$data['tgl']??'',$data['alamat']??'',$data['hp']??'',$data['jalur']??'',$data['ijazah']??null,$data['akte']??null,$data['tanggal_daftar']??date('d/m/Y')]);
    echo json_encode(['ok'=>true]);
    break;
...
case 'delete_ppdb':
    requireAuth();
    $id = (int)($input['id'] ?? 0);
    $p = $pdo->prepare("SELECT nama FROM ppdb WHERE id=?"); $p->execute([$id]); $row = $p->fetch();
    $pdo->prepare("DELETE FROM ppdb WHERE id=?")->execute([$id]);
    addLog($pdo, 'Hapus PPDB', 'Menghapus pendaftar: '.($row['nama']??''));
    echo json_encode(['ok'=>true]);
    break;
...
case 'clear_ppdb':
    ...
    addLog($pdo, 'Clear PPDB', 'Menghapus semua data pendaftar');
    echo json_encode(['ok'=>true]);
    break;
...
case 'save_biaya':
    requireAuth();
    $gambar = $input['gambar'] ?? '';
    setSetting($pdo, 'biaya_gambar', $gambar);
    addLog($pdo, 'Upload Biaya', 'Mengupload gambar pamplet biaya');
    echo json_encode(['ok'=>true]);
    break;
...
case 'delete_biaya':
    ...
    addLog($pdo, 'Hapus Biaya', 'Menghapus gambar pamplet biaya');
    echo json_encode(['ok'=>true]);
    break;
...
case 'get_users':
    requireAdmin();
    $rows = $pdo->query("SELECT id,username,role,status FROM users ORDER BY id")->fetchAll();
...
case 'add_user':
    ...
    if (!$username || !$password) { echo json_encode(['error'=>'Username dan password wajib diisi']); break; }
    try {
        ...
        addLog($pdo, 'Tambah User', "Menambah user: $username ($role)");
        echo json_encode(['ok'=>true]);
    } catch (Exception $e) {
...
case 'edit_user':
    ...
    $role     = $input['role'] ?? 'staff';
    $status   = $input['status'] ?? 'aktif';
    if (!$username) { echo json_encode(['error'=>'Username wajib diisi']); break; }
    if ($password) {
        $pdo->prepare("UPDATE users SET username=?,password=?,role=?,status=? WHERE id=?")
            ->execute([$username, password_hash($password, PASSWORD_DEFAULT), $role, $status, $id]);
    } else {
        $pdo->prepare("UPDATE users SET username=?,role=?,status=? WHERE id=?")
            ->execute([$username, $role, $status, $id]);
    }
    addLog($pdo, 'Edit User', "Mengedit user: $username");
    echo json_encode(['ok'=>true]);
    break;
...
case 'delete_user':
    ...
    $u = $pdo->prepare("SELECT username FROM users WHERE id=?"); $u->execute([$id]); $row = $u->fetch();
    $pdo->prepare("DELETE FROM users WHERE id=?")->execute([$id]);
    addLog($pdo, 'Hapus User', 'Menghapus user: '.($row['username']??''));
    echo json_encode(['ok'=>true]);
    break;
...
case 'toggle_user':
    requireAdmin();
    $id = (int)($input['id'] ?? 0);
    $u = $pdo->prepare("SELECT * FROM users WHERE id=?"); $u->execute([$id]); $row = $u->fetch();
    if ($row) {
        $newStatus = $row['status'] === 'aktif' ? 'nonaktif' : 'aktif';
        $pdo->prepare("UPDATE users SET status=? WHERE id=?")->execute([$newStatus, $id]);
        addLog($pdo, 'Toggle Status User', "User {$row['username']} menjadi $newStatus");
...
case 'clear_logs':
    ...
    addLog($pdo, 'Clear Logs', 'Menghapus semua log aktivitas');
    echo json_encode(['ok'=>true]);
    break;
